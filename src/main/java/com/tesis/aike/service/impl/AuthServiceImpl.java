...
public class AuthServiceImpl implements AuthService {
    ...
    @Override
    @Transactional
    public Map<String, Object> loginGoogleWeb(String idToken) {
        log.info("Iniciando proceso de login con Google.");
        FirebaseToken firebaseToken;
        try {
            firebaseToken = googleTokenVerifierService.verifyWebToken(idToken);
    ...
    @Override
    @Transactional
    public Map<String, Object> loginGoogleAPK(String idToken) {
        log.info("Iniciando proceso de login con Google.");
        GoogleIdToken.Payload payload;
        try {
            payload = googleTokenVerifierService.verifyApkToken(idToken);
            if (payload == null) {
                log.error("La verificación del token de Google ha fallado.");
                throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "ID token inválido");
            }
